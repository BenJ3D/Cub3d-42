...
void	ft_make_bonus_img(t_main *main)
{
	main->gm.img_door.img = mlx_xpm_file_to_image(main->mlx, IMG_DOOR, \
	&main->gm.img_door.width, &main->gm.img_door.height);
	main->gm.img_door.addr = mlx_get_data_addr(main->gm.img_door.img, \
	&main->gm.img_door.bpp, &main->gm.img_door.line_length, \
														&main->gm.img_door.end);
